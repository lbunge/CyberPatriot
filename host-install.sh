...
apt install -y xfce4 slim lxpolkit wget gcc build-essential linux-headers-generic linux-headers-$(uname -r) firefox
...
sudo -u $newUser vncserver -geometry 1920x1080
lxpolkit &  # start policy authentication agent in background as a job
disown -h   # disown lxpolkit pid so the script terminal can close